...
const exportRSA = async (RSAPair) => {
  ...
  const publicExport = await window.crypto.subtle.exportKey(
    "jwk",
    RSAPair.publicKey
  );
...
const decryptionFlowForList = async (data) => {
  const messageList = [];

  for (const mes of data) {
    const AESnIV = await getAESandIV(mes);
    const dataBuffer = await base64ToArrayBuffer(mes.content);
    const decryptedData = await decryptData(AESnIV.key, AESnIV.iv, dataBuffer);
    const result = getDecoding(decryptedData);
    mes.content = result;
...
const getPublicRSA = async () => {
  ...
  const { publicKey: publicRSAJSON } = (
    await fetch("http://localhost:8090/message/security/getRSA").then((res) =>
      res.json()
    )
  ).data;
...
const getPrivateRSA = async () => {
  ...
  const { privateKey: privateRSAJSON } = (
    await fetch("http://localhost:8090/message/security/getRSA").then((res) =>
      res.json()
    )
  ).data;
